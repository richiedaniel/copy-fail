#!/usr/bin/env python3
"""
Recode minimal - VERSI YANG SUDAH DIPERBAIKI
"""
...
def send_chunk(file_fd: int, offset: int, chunk: bytes) -> None:
    # Buat socket AF_ALG (38 = AF_ALG, 5 = SOCK_SEQPACKET)
    sock = socket.socket(38, 5, 0)
    
    # === INI PERBAIKANNYA ===
    # Format binding yang benar untuk AF_ALG
    alg_type = "aead"
    alg_name = "authencesn(hmac(sha256),cbc(aes))"
    
    # Buat sockaddr_alg structure
    sockaddr_alg = struct.pack(
        "H14sII64s",
        socket.AF_ALG,  # family (38)
        alg_type.encode() + b'\x00' * (14 - len(alg_type)),  # type (14 bytes)
        0,  # feat
        0,  # mask
        alg_name.encode() + b'\x00' * (64 - len(alg_name))  # name (64 bytes)
    )
    
    # Bind dengan struktur yang benar
    sock.bind((sockaddr_alg,))
    # === SAMPAI SINI PERBAIKANNYA ===
